<script>
    ...
    function handleClick2() {
		const studentObj = studentData.find((student) => student.name == nameSelected2);
		const topicObj = topicData.find((topic) => topic.topic == topicSelected2);
		const courseData = {
			name: nameSelected2,
			module: moduleSelected2,
            topic: topicSelected2,
			topicimage: topicObj.topicimage,
			profileimage: studentObj.profileimage,
			slackmsg: studentObj.slackhuddle,
			slackhuddle: topicObj.topichuddle
...
<div class="container mx-auto p-8 space-y-8">
	...
	<form id="studentForm">
		<div class="card p-4 grid grid-cols-1 lg:grid-cols-3 gap-4">
			<label class="label">
				<span> Name</span>
				<select id="name" bind:value={nameSelected2} on:change={() => (name = '')} class="select">
					{#each studentData as student}
						<option value={student.name}>
							{student.name}
						</option>
			...
			<label class="label">
				<span> Topic</span>
				<select id="topic" bind:value={topicSelected2} on:change={() => (topic = '')} class="select">
					{#each topicData as topic}
						<option value={topic.topic}>
							{topic.topic}
						</option>
